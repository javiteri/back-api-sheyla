...
async function generateHeaderPDF(pdfDoc, datosEmpresa, datosCliente, datosVenta){
    ...
    const pathImagen = await getImagenByRucEmp(datosEmpresa[0]['EMP_RUC']);

    let fontNormal = 'Helvetica';
    let fontBold = 'Helvetica-Bold';
    
    if(pathImagen){
        console.log('inside path image');
        pdfDoc.image(pathImagen,50,50,{fit: [150, 100],align: 'center', valign: 'center'});
    }
    
    pdfDoc.fontSize(9);
    pdfDoc.font(fontBold).text(datosEmpresa[0]['EMP_NOMBRE'], 20, 170, {width: 250});
    ...
    pdfDoc.text(`BARCODE IMAGE`, 280, 240);
    pdfDoc.text(`${digit48}`, 280, 250);
